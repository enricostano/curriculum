...
<xsl:comment>
  ...
  # URL       : http://europass.cedefop.europa.eu/TechnicalResources/XML/xsl/europass-xsl-previous/cv_es_ES.xsl
</xsl:comment>
...
<xsl:template name="personalinformation">
	...
	<xsl:if test="//field[@name='step1.gender' and @keep='true']">
		<xsl:call-template name="labelRow">
			...
			<xsl:with-param name="labelcontent">
				<xsl:if test="//identification/demographics/gender='M'"><xsl:text>Masculino</xsl:text></xsl:if>
			  <xsl:if test="//identification/demographics/gender='F'"><xsl:text>Femenino</xsl:text></xsl:if>
			</xsl:with-param>
		</xsl:call-template>
	</xsl:if>
</xsl:template>
